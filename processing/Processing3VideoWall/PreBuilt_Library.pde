...
void wallSetup() {
    ...
    surface.setSize(int(wallWidth*_wallScale), int(wallHeight*_wallScale)); // use surface instead of frame and input width and height variables only
    ...
    surface.setTitle("Running " + _wallNames[_wallType-1] + " emulation at " + str(width) + "px x " + str(height) + "px (scaling factor of " + _wallScale + ")"); // FOR VERSION 3.x, use surface instead of frame
